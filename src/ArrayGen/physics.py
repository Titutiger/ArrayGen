...
class Physics:
    ...
    @staticmethod
    def vector_magnitude(vector: str) -> float:
        """
        Computes the magnitude of a 2D or 3D vector given in i, j, k string notation.

        Parameters
        ----------
        vector : str
            A string like '3i+6j-3k' or '4i-2j'.
        ...
        """
        matches = re.findall(r'([+-]?\d*)\s*([ijk])', vector.replace(' ', ''))
        vals = {'i': 0, 'j': 0, 'k': 0}
        for coef, axis in matches:
            if coef in ('', '+', '-'):
                val = 1 if coef in ('', '+') else -1
            else:
                val = int(coef)
            vals[axis] += val
        comp = [vals[a] for a in ['i', 'j', 'k']]
        # Use only nonzero components
        nonzero = [c for c in comp if c != 0 or vector.find('k') >= 0]
        return math.sqrt(sum(x ** 2 for x in nonzero))
